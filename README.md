...
- **SR Portraits**  
  Shows the speaking token portrait next to chat speaker names and opens the actor sheet when the portrait is clicked. It refreshes visible portraits when token, actor, or user images change, handles wildcard token image paths safely, and falls back to the player/GM avatar for messages without a usable actor image. Requires the Shadowrun 5 system.
